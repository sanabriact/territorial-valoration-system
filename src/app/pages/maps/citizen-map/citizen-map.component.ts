...
import { AnnotationMapComponent } from '../annotation/components/annotation-map/annotation-map.component';
import { AnnotationMarker, NeighborhoodPolygon } from '../annotation/models/annotation-map.model';

@Component({
  selector: 'app-citizen-map',
  standalone: true,
  imports: [CommonModule, FormsModule, AnnotationMapComponent],
  templateUrl: './citizen-map.component.html',
  styleUrl: './citizen-map.component.scss',
})
export class CitizenMapComponent implements OnInit {
  ...
  private loadInitialData(): void {
    ...
    forkJoin({
      ...
      currentCitizenId: this.currentCitizenService.getCurrentCitizenId(),
    })
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: ({ communes, neighborhoods, categories, points, annotations, annotationCategories, currentCitizenId }) => {
          ...
          this.currentCitizenId.set(currentCitizenId);
  ...
  private getSaveErrorMessage(error: unknown): string {
    if (error instanceof HttpErrorResponse) {
      ...
      return backendMessage || `No se pudo guardar la calificacion. Codigo HTTP: ${error.status}`;
    }

    return 'No se pudo guardar la calificacion.';
  }
}
